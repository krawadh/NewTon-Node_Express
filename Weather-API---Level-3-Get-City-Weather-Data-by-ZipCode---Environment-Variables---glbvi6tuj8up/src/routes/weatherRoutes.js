...
router.get('/city/zipcode/:code', async (req, res) => {
   ...
   const zip = req.params.code;
   try{
    const forecastRs = await weatherController.getWeatherDataByZipCode(zip);
    res.status(200).send({"status": "success", "message": "Weather data retrieved", "data": forecastRs})
   }catch{
    res.status(404).send({"status": "error","message": "ZipCode not found", 'error': 'ZipCode not found'});
   }
