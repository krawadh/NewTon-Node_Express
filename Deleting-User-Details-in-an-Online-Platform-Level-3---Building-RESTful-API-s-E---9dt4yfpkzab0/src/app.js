...
app.delete('/api/v1/details/:id', (req, res) => {
  const index = userDetails.findIndex(e => e.id === parseInt(req.params.id));
  if (index === -1) {
    ...
    fs.writeFile(`${__dirname}/data/userDetails.json`, JSON.stringify(userDetails), (err) => {
      ...
      return res.status(200).json({ status: "success", message: "User details deleted successfully", data: details});
    })
  }
})
