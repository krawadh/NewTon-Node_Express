...
router.post('/alerts', async (req, res) => {
   ...
   try{
    const writefile = await weatherController.saveWeatherAlert(bodyData);
    res.status(200).send({"status": "success", "message": "Weather alert saved successfully"});
   }catch{
    res.status(200).send({"status": "error", "message": "Failed to save weather alert"});
   }
});
